...
main(int argc, char **argv)
{
	...
	/* Setup signal handlers to terminate the event loop */
	...
	if (lport < 1024 || lport > 65535 || rport > 65535) {
	 ...
	 * - to force the OS to use that port as source port when sending. */
	...
	/* Event loop */
	while (run) {
		/* Initializing everything to safe values is a good idea */
		memset(buffer, 0, sizeof(buffer));
		memset(&safrom, 0, sizeof(safrom));
		...
		if (0 > ret) {
			if (errno == EINTR)	// that's ok
				continue;

			perror("select() failed");
			exit(1);
		}
		if (0 == ret) {			// should not happen here
